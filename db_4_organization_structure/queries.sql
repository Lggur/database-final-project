...
-- Задача 2
SELECT 'Дерево сотрудников со статистикой задач и прямых подчиненных' AS 'Задача 2';    
WITH RECURSIVE employee_hierarchy AS (
    SELECT
        EmployeeID,
        Name,
...
-- Задача 3
SELECT 'Поиск и анализ структуры менеджмента организации, а также расчет количества вложенных подчиненных для каждого сотрудника с должностью Менеджер' AS 'Задача 3';
WITH RECURSIVE employee_hierarchy AS (
    SELECT
        e.EmployeeID,
        e.Name,
